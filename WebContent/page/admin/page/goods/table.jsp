...
<div class="container-fluid">
	<div class="row-fluid">
		<div class="span12">
			...
			<form class="form-search" action="${basePath}page/goodsServlet" method="get">
				<input type="hidden" name="method" value="getPage">
				<input class="input-medium search-query" type="text" name="goods_name" value="${param.goods_name}" /> <button type="submit" class="btn">查找</button>
			</form><br/> <button class="btn btn-primary" type="button" onclick="window.location.href='${basePath}page/admin/page/gclass/gclassServlet?method=all';">添加</button> <button class="btn btn-danger" type="button" onclick="remove();">批量删除</button>
			<br/>
			<table class="table table-bordered table-hover">
				...
				<tbody>
					...
					<tr>
						<td>
							<input type="checkbox" name="goods_ids" value="${goodsBean.goods_id }" >
						</td>
						<td>
							${in.index+1+(pageUtil.current-1)*10}
						</td>
						<td>
							<img width="100px" height="100px" src="${basePath }upload/${goodsBean.goods_image}">
						</td>
						<td>
							${goodsBean.goods_name}
						</td>
						<td>
							${goodsBean.goods_price}￥
						</td>
						<td>
							${goodsBean.goods_desc}
						</td>
						<td>
							<a href="${basePath}page/goodsServlet?method=findById&goods_id=${goodsBean.goods_id}"><img src="${basePath}common/admin/images/修改.png">修改</a>
							<a href="${basePath}page/goodsServlet?method=delete&goods_ids=${goodsBean.goods_id}"><img src="${basePath}common/admin/images/删除／数字面板编辑态.png">删除</a>
						</td>
					</tr>
				</c:forEach>
				</tbody>
...
<script type="text/javascript">
	function remove(){
		...
		var goods_ids=document.getElementsByName("goods_ids");
		var goods_url = "${basePath}page/goodsServlet?method=delete";
		
		for (var i = 0; i < goods_ids.length; i++) {
			
			if(goods_ids[i].checked==true){
				
				goods_url+="&goods_ids="+goods_ids[i].value;
			}
		}
